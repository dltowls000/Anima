...
using TMPro;
using UnityEngine;
public class SingleAttackSkill:MonoBehaviour
{
    IBattleManager bm;
    public SingleAttackSkill(IBattleManager bm)
    {
        this.bm = bm;
    }
            ...
            foreach (AnimaActions anima in bm.AllyActions)
            {
                ...
                if (ReferenceEquals(bm.TurnList[0], anima.animaData))
                {
                    ...
                    if (bm.EnemyActions[selectEnemy].animaData.Animadie)
                    {
                        ...
                        bm.EnemyBattleSetting.animator.RemoveAt(selectEnemy);
                        ...
                        if (bm.EnemyActions.Count == 0)
                        {
                            ...
                            bm.stat = BattleManager.State.win;
                            bm.TurnIndex = 0;
                            bm.WinBattle();
